...
class CoinbaseWallet {
  ...
  async connect() {
    if (!this.isInstalled) {
      window.open(WALLET_URLS.ethereum.coinbase, '_blank');
      throw new Error('Coinbase Wallet not installed. Please install and refresh the page.');
    }
    ...
    try {
      ...
      if (error.code === -32002) {
        throw new Error('Connection request pending. Please check Coinbase Wallet.');
      }
      throw error;
    }
  }
  ...
  async switchNetwork(chainId) {
    ...
    try {
      ...
      if (error.code === 4902) {
        throw new Error('Network not added to Coinbase Wallet');
      }
      throw error;
    }
  }
